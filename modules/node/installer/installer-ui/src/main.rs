...
fn zoneinfo_tab_candidates(names: &[&str]) -> Vec<PathBuf> {
	...
	if let Some(base_dir) = std::env::var_os("NODE_INSTALLER_ZONEINFO_DIR") {
...
fn resolve_installer_runner() -> Result<String, AppError> {
	let runner = std::env::var("NODE_INSTALLER_RUNNER")
		.ok()
...
async fn post_install(
	...
	tokio::spawn(async move {
		...
		cmd.env("NODE_I_UNDERSTAND_THIS_WIPES_DISKS", "YES");
		...
		if let Some(error) = spawn_error {
			next_state.last_error = Some(format!("Falha ao iniciar node-install: {error}"));
...
async fn main() -> Result<(), AppError> {
	...
	let listen = std::env::var("NODE_INSTALLER_LISTEN").unwrap_or_else(|_| "0.0.0.0:8000".into());
	let addr: SocketAddr = listen
		.parse()
		.map_err(|_| AppError::Validation("NODE_INSTALLER_LISTEN inválido".into()))?;

	let static_dir = std::env::var("NODE_INSTALLER_STATIC")
		.map(PathBuf::from)
		.unwrap_or_else(|_| {
	...
	let imgs_dir = std::env::var("NODE_INSTALLER_IMGS")
		.map(PathBuf::from)
		.unwrap_or_else(|_| {
	...
	let runtime_dir = std::env::var("NODE_INSTALLER_RUNTIME")
		.map(PathBuf::from)
		.unwrap_or_else(|_| {
			let mut candidates: Vec<PathBuf> = vec![PathBuf::from("/run/node-installer")];
			if let Ok(xdg) = std::env::var("XDG_RUNTIME_DIR") {
				if !xdg.trim().is_empty() {
					candidates.push(PathBuf::from(xdg).join("node-installer"));
				}
			}
	...
	info!(%addr, "node-installer-ui listening");
