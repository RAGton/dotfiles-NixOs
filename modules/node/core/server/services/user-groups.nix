...
#   - Grupos adicionais: criados dinamicamente via CLI `node group add`
...
#   ├── <grupo1>/       (criado via: node group add)
#   ├── <grupo2>/
#   └── .archive/       (armazena grupos deletados)
#
# CLI para gerenciar:
#   node group add <nome> [--storage-quota 100G]
#   node group list
#   node group delete <nome> --archive
#   node group chmod <nome> <perms>         (ex: 0750, g+rw)
#   node group members <nome> [--add USER] [--remove USER]
#   node group permissions <nome>           (listar permissões atuais)
#
# Permissões Padrão:
...
in
{
  options.node.userGroups = {
    enable = mkEnableOption "NODE user groups and sector storage (dynamic creation via CLI)";
  };

  config = mkIf config.node.userGroups.enable {
    # -----------------------------------------------------------------------
