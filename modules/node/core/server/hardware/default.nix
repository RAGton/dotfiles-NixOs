/*
  Hardware do servidor NODE.

  IMPORTANTÍSSIMO:
  - Nada de UUIDs/discos aqui.
  - O hardware real é gerado pelo instalador via `nixos-generate-config --root /mnt`
    e persistido em `/var/lib/node/runtime/hardware-configuration.nix`
    antes do `nixos-install`.
  - O checkout em `/etc/node` pode manter apenas links de compatibilidade para leitura humana.
