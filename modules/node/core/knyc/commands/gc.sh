cmd_gc_impl() {
  ...
  local snapshots_root="${KNYC_SNAPSHOTS_ROOT:-/srv/data/snapshots}"
  local snapshot_keep="${KNYC_GC_SNAPSHOT_KEEP:-7}"
...
cmd_gc() {
  with_global_lock "knyc gc" cmd_gc_impl "${1:-$KEEP_VERSIONS}" "${2:-}"
}
