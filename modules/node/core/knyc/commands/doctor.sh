cmd_doctor() {
  log_section "NODE Infrastructure Check"
  echo ""
  ...
  if pointer_exists current; then
    ...
    if command -v nix-store >/dev/null 2>&1 && [[ "${KNYC_ALLOW_NON_NIX_STORE_PATHS:-0}" != "1" ]]; then
  ...
  local grace_seconds="${KNYC_GC_GRACE_SECONDS:-900}"
  local now
  now="$(date +%s)"
  for dir in "$IMAGES_ROOT"/v*; do
    [[ -d "$dir" ]] || continue
  ...
  if [[ "$DOCTOR_FAIL" -eq 0 ]]; then
    log_ok "NODE server healthy"
    return 0
  fi

  log_error "NODE server has issues ($DOCTOR_FAIL falha(s))"
  echo ""
  echo "Dicas:"
  echo "  - Verifique servicos: systemctl status dnsmasq nginx nfs-server"
  echo "  - Execute knyc list e valide current/previous/staged"
