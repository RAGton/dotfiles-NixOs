KNYC_ROLLBACK_PENDING_PATH="${KNYC_ROLLBACK_PENDING_PATH:-$IMAGES_ROOT/.rollback.pending}"
KNYC_LAST_ROLLBACK_PATH="${KNYC_LAST_ROLLBACK_PATH:-$IMAGES_ROOT/.rollback.last}"
...
ensure_runtime_contract_for_publish() {
  [[ -n "${KNYC_TEST_SYSTEM_PATH:-}" ]] && return 0

  local runtime_root="${NODE_RUNTIME_ROOT:-/var/lib/node/runtime}"
...
target_installable() {
  ...
  # Checkouts operacionais materializados em /etc/node podem manter .git/.gitmodules
  ...
  case "$resolved_target" in
    desktop-generic)
      printf '%s\n' "$flake_ref#nixosConfigurations.node-client-desktop-generic.config.system.build.nodePublishTree"
      ;;
    desktop-lab)
      printf '%s\n' "$flake_ref#nixosConfigurations.node-client-desktop-lab.config.system.build.nodePublishTree"
      ;;
    hyperv-debug)
      printf '%s\n' "$flake_ref#nixosConfigurations.node-client-hyperv-debug.config.system.build.nodePublishTree"
      ;;
    rescue-minimal)
      printf '%s\n' "$flake_ref#nixosConfigurations.node-client-rescue-minimal.config.system.build.nodePublishTree"
      ;;
  esac
}

storage_mount_ready() {
  [[ "${KNYC_SKIP_STORAGE_CHECKS:-0}" == "1" ]] && return 0
...
build_or_reuse_system() {
  local flake_root="$1"
  local requested_target="$2"
  local result_link="$flake_root/.knyc-build-result"

  rm -f "$result_link"
  trap_add "rm -f '$result_link'"

  if [[ -n "${KNYC_TEST_SYSTEM_PATH:-}" ]]; then
    printf '%s\n' "$KNYC_TEST_SYSTEM_PATH"
    return 0
  fi
...
write_pending_rollback() {
  ...
  local tmp="${KNYC_ROLLBACK_PENDING_PATH}.tmp.$$"
...
EOF
  mv -f "$tmp" "$KNYC_ROLLBACK_PENDING_PATH"
}

load_pending_rollback() {
  [[ -f "$KNYC_ROLLBACK_PENDING_PATH" ]] || return 1
  # shellcheck disable=SC1090
  source "$KNYC_ROLLBACK_PENDING_PATH"
  [[ -n "${source:-}" && -n "${target:-}" ]]
}

clear_pending_rollback() {
  rm -f "$KNYC_ROLLBACK_PENDING_PATH"
}
...
write_last_rollback() {
  ...
  local target_ver="$2"
  local channel_name="${3:-}"
  local tmp="${KNYC_LAST_ROLLBACK_PATH}.tmp.$$"
...
channel=$channel_name
EOF
  mv -f "$tmp" "$KNYC_LAST_ROLLBACK_PATH"
}

load_last_rollback() {
  [[ -f "$KNYC_LAST_ROLLBACK_PATH" ]] || return 1
  # shellcheck disable=SC1090
  source "$KNYC_LAST_ROLLBACK_PATH"
  [[ -n "${source:-}" && -n "${target:-}" ]]
}

clear_last_rollback() {
  rm -f "$KNYC_LAST_ROLLBACK_PATH"
}
