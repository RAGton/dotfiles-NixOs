...
isset \${ip} || dhcp
echo Booting NODE $channel (\${build_id})...
kernel http://$SERVER_IP:$HTTP_PORT/netboot/$boot_dir/bzImage init=$init_path ip=dhcp node.primaryNicMac=\${net0/mac} $kernel_params
...
echo Publique com: knyc switch --channel $channel
shell
IPXE
  fi
}
...
isset \${ip} || dhcp
echo Booting NODE current (\${build_id})...
kernel http://$SERVER_IP:$HTTP_PORT/netboot/current/bzImage init=$current_init ip=dhcp node.primaryNicMac=\${net0/mac} $current_params
...
isset \${ip} || dhcp
echo Booting NODE rescue (\${build_id})...
kernel http://$SERVER_IP:$HTTP_PORT/netboot/rescue/bzImage init=$rescue_init ip=dhcp node.primaryNicMac=\${net0/mac} $rescue_params
