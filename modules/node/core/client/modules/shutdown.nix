...
let
  homeUnmountScript = pkgs.writeShellScript "node-home-unmount" ''
        set -eu

        log() {
          echo "node-shutdown: $*" >&2
        }
...
in
{
  systemd.services.node-home-unmount = {
    description = "Desmonta homes NFS do NODE antes da parada da rede";
    wantedBy = [
      "halt.target"
      "reboot.target"
      "poweroff.target"
