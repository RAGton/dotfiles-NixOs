...
let
  cfg = config.node.auth.pamMountUserGroups;
  ...
  pamUserSectorMounts = pkgs.writeShellScriptBin "node-user-sector-mounts" ''
    set -euo pipefail

    pam_user="''${PAM_USER:-}"
    pam_type="''${PAM_TYPE:-open_session}"
    node_server="${cfg.server}"
    node_storage_remote="${cfg.remoteBase}"
    node_sectors_dir_name="${cfg.sectorsDirName}"
    ...
    log_info() {
      logger -t node-user-sector-mounts -p user.info "[$pam_user][$pam_type] $*"
    }

    log_error() {
      logger -t node-user-sector-mounts -p user.err "[$pam_user][$pam_type] $*"
    }
    ...
    ensure_sectors_base() {
      local user_home="$1"
      local sectors_base="$user_home/$node_sectors_dir_name"
      local primary_group
    ...
    mount_user_groups() {
      ...
      for group_name in $(id -Gn "$pam_user"); do
        ...
        if mount -t nfs4 \
          -o "$mount_options" \
          "$node_server:$node_storage_remote/$group_name" \
    ...
    umount_user_groups() {
      ...
      sectors_base="$user_home/$node_sectors_dir_name"
...
in
{
  options.node.auth.pamMountUserGroups = {
  ...
  config = lib.mkIf cfg.enable {
    security.pam.services.login.rules.session.nodeUserSectorMounts = {
      order = config.security.pam.services.login.rules.session.mount.order + 10;
      control = "optional";
      modulePath = "${config.security.pam.package}/lib/security/pam_exec.so";
      args = [ "${pamUserSectorMounts}/bin/node-user-sector-mounts" ];
    };

    security.pam.services.sddm.rules.session.nodeUserSectorMounts = {
