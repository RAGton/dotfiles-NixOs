...
let
  ...
  adminSpec = {
    ...
    authorizedKeys = nodeAdminAuthorizedKeys;
    extraGroups = [ "wheel" ] ++ defaultUserGroups;
  };

  runtimeUsers = lib.mapAttrs (
    name: spec: if name == nodeAdminUser then adminSpec // spec else spec
  ) nodeClientUsers;

  managedUsers =
    lib.optionalAttrs (nodeAdminUser != "" && nodeAdminUser != null) {
      "${nodeAdminUser}" = adminSpec;
    }
    // runtimeUsers;
  ...
  mkUser = name: spec: {
    isNormalUser = true;
    description =
      if spec ? description && spec.description != "" then spec.description else "NODE User";
    uid =
      if spec ? uid then
        spec.uid
      else if name == nodeAdminUser then
        nodeAdminUid
      else
        throw "NODE: usuario ${name} precisa declarar uid no catalogo clientUsers";
    home = "/home/${name}";
