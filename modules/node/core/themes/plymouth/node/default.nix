...
stdenvNoCC.mkDerivation {
  ...
  installPhase = ''
    ...
    themeDir="$out/share/plymouth/themes/node"
    ...
    if [[ -f background.png ]]; then
      ...
      echo "plymouth-theme-node: missing background asset (expected background.png, background.jpg or source-background.jpg)" >&2
      exit 1
    fi

    install -Dm0644 node.plymouth "$themeDir/node.plymouth"
    install -Dm0644 node.script "$themeDir/node.script"
    ...
    substituteInPlace "$themeDir/node.plymouth" \
      --replace-fail "@PLYMOUTH_THEME_DIR@" "$themeDir"
    substituteInPlace "$themeDir/node.script" \
  ...
  meta = with lib; {
    description = "Tema Plymouth autoral do NODE para boot netboot";
