#!/usr/bin/env python3
"""Generate the NODE Plasma wallpaper collections.
...
slides with a restrained technical landscape and a subtle NODE mark.
"""
...
def extract_logo(logo_source: Path) -> Image.Image:
    ...
    bbox = mask.getbbox()
    if not bbox:
        raise RuntimeError("Failed to extract NODE logo alpha mask.")
