...
let
  cfg = config.kryonix.services.node-think;
  # Importar o helper
  inherit (import ../../../lib/services.nix { inherit lib; }) mkKryonixService;
in
{
  options.kryonix.services.node-think = {
    enable = lib.mkEnableOption "NODE Think Service";

    serverIp = lib.mkOption {
      type = lib.types.str;
      default = "127.0.0.1";
      description = "IP primário do servidor NODE.";
    };
  ...
  config = lib.mkIf cfg.enable (mkKryonixService {
    ...
    legacyLinks = {
      "/var/lib/node/runtime/params.nix" = "params.nix";
    }
    // lib.optionalAttrs (cfg.wanMode == "pppoe") {
      "/etc/node/wan-pppoe.env" = "wan-pppoe.env";
    };
  });
}
