...
let
  debugScript = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      cat /run/ragos/boot-network.env 2>/dev/null || echo "absent"
      echo
...
in
{
  ...
  systemd.services.ragos-boot-report = {
    description = "Registra resumo do boot diskless do cliente RAGOS";
    after = [
    ...
    serviceConfig = {
      Type = "oneshot";
      ExecStart = "${debugScript}/bin/ragos-client-debug";
