{
  ...
  ragosInventoryDir ? "/etc/ragos-inventory",
  ragosInventoryRequireNonEmpty ? true,
  ...
}:

let
  # Subnet /24 derivada do IP do servidor (ex: 192.168.100.10 → 192.168.100.0/24)
  lanOctets = lib.strings.splitString "." ragosServerIp;
  ...
  inventoryFile = "${ragosInventoryDir}/clients.nix";
  inventoryRuntimeDir = "/run/ragos-inventory";
  ...
  renderInventoryScript = pkgs.writeShellApplication {
    ...
    text = ''
            ...
            if [[ ! -f "$inventory_path" ]]; then
              ${
                if ragosInventoryRequireNonEmpty then
                  ''
                    echo "RAGOS: inventario externo ausente em $inventory_path" >&2
                    echo "RAGOS: crie $inventory_path e mantenha o inventario em /etc/ragos-inventory." >&2
                    exit 1
                  ''
                else
                  ''
            ...
            inventory_json="$(
              nix-instantiate \
                ...
                --arg requireNonEmpty ${if ragosInventoryRequireNonEmpty then "true" else "false"} \
                --expr '
            ...
            {
              printf '# gerado por ragos-render-inventory\n'
              printf '# source=%s\n' "$inventory_path"
      ...
      chain --replace http://${ragosServerIp}:${toString ragosHttpPort}/''${channel}.ipxe || goto failed
  ...
  applyInventoryScript = pkgs.writeShellApplication {
    ...
    text = ''
      set -euo pipefail
      ragos-render-inventory ${lib.escapeShellArg inventoryFile} ${lib.escapeShellArg inventoryDnsmasqHostsFile}
...
in
{
  assertions = [
    {
      assertion = ragosMgmtPrefixLength == 24;
      message = "RAGOS: DHCP declarativo do srv-rag atualmente exige mgmtPrefixLength=24 para evitar drift silencioso de rede.";
    }
  ];
  ...
  environment.etc."ragos-inventory/clients.nix".source = "/var/lib/ragos/inventory/clients.nix";
  environment.etc."ragos-inventory/README.md".source = ../network/clients-inventory.README.md;
  environment.etc."ragos-inventory/clients.template.nix".source =
    ../network/clients-inventory.bootstrap.nix;

  warnings =
    lib.optional (!ragosInventoryRequireNonEmpty)
      "RAGOS: inventario externo vazio foi explicitamente permitido. Isso reduz a protecao fail-closed do cadastro de clientes.";
  ...
  services.dnsmasq = {
    ...
    settings = {
      ...
      dhcp-boot = [
        "tag:ipxe,http://${ragosServerIp}:${toString ragosHttpPort}/boot.ipxe"
  ...
  systemd.tmpfiles.rules = [
    ...
    "d /var/lib/ragos/inventory 0755 root root -"
    "f /var/lib/ragos/inventory/clients.nix 0644 root root -"
    "d ${inventoryRuntimeDir} 0755 root root -"
    "d ${inventoryIpxeMacDir} 0755 root root -"
  ];

  system.activationScripts.ragosInventory = lib.stringAfter [ "etc" ] ''
        inventory_dir=${lib.escapeShellArg ragosInventoryDir}
        inventory_file=${lib.escapeShellArg inventoryFile}
        require_non_empty="${if ragosInventoryRequireNonEmpty then "1" else "0"}"
    ...
    EOF
        ...
        echo "RAGOS: validando inventario externo em ${inventoryFile}" >&2
        ${renderInventoryScript}/bin/ragos-render-inventory ${lib.escapeShellArg inventoryFile} ${lib.escapeShellArg inventoryDnsmasqHostsFile}
  '';
