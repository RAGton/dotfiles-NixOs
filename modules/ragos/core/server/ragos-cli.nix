...
pkgs.writeShellApplication {
  ...
  text = ''
        ...
        readonly RAGOS_FLAKE_PATH="''${RAGOS_FLAKE_PATH:-/etc/ragos}"
        readonly RAGOS_TARGET_HOST="''${RAGOS_TARGET_HOST:-srv-rag}"
        readonly RAGOS_INSTALLABLE="git+file://''${RAGOS_FLAKE_PATH}#''${RAGOS_TARGET_HOST}"
        readonly RAGOS_RUNTIME_ROOT="''${RAGOS_RUNTIME_ROOT:-/var/lib/ragos/runtime}"
        readonly RAGOS_SYSTEM_PROFILE="/nix/var/nix/profiles/system"
        readonly RAGOS_KEEP_GENERATIONS=5
        readonly RAGOS_KEEP_SINCE="7d"
        readonly RAGOS_KEEP_SINCE_DATE_EXPR="7 days ago"
        readonly RAGOS_HOME_BASE="/srv/data/home"
        readonly RAGOS_HOME_ARCHIVE_BASE="/srv/data/home/.archive"
        readonly RAGOS_HOME_SNAPSHOT_BASE="/srv/data/snapshots/users"
        readonly RAGOS_HOME_META_FILE=".ragos-home-meta"
        readonly RAGOS_CLIENT_USERS_FILE="$RAGOS_RUNTIME_ROOT/client-users.json"
        readonly RAGOS_STORAGE_BASE="/srv/data/storage"
        readonly RAGOS_STORAGE_ARCHIVE="/srv/data/storage/.archive"
        readonly RAGOS_AUDIT_DIR="/var/lib/ragos/audit"
        readonly RAGOS_AUDIT_FILE="$RAGOS_AUDIT_DIR/login-history.json"
        readonly RAGOS_USER_GROUPS_FILE="$RAGOS_RUNTIME_ROOT/user-groups.json"
        ...
        require_flake_dir() {
          [[ -d "$RAGOS_FLAKE_PATH" ]] || die "flake local ausente em $RAGOS_FLAKE_PATH"
        }

        require_git_repo() {
          have_cmd git || die "git nao disponivel no PATH"
          git -C "$RAGOS_FLAKE_PATH" rev-parse --is-inside-work-tree >/dev/null 2>&1 \
            || die "checkout operacional nao e um repositorio Git funcional: $RAGOS_FLAKE_PATH"
        }

        require_runtime_file() {
          local file_name="$1"
          local path="$RAGOS_RUNTIME_ROOT/$file_name"
          [[ -f "$path" ]] || die "RAGOS runtime ausente: srv-rag exige runtime local valido em $RAGOS_RUNTIME_ROOT (faltando $file_name)"
        }

        resolve_boot_device() {
          nix eval --impure --raw --expr \
            "let flake = builtins.getFlake \"git+file://$RAGOS_FLAKE_PATH\"; in flake.nixosConfigurations.\"$RAGOS_TARGET_HOST\".config.fileSystems.\"/boot\".device"
        }
        ...
        validate_runtime_guard() {
          ...
          [[ -n "$boot_device" ]] || die "RAGOS runtime invalido: nao foi possivel resolver /boot para $RAGOS_INSTALLABLE"

          case "$boot_device" in
            /dev/disk/by-label/ESP|/dev/disk/by-label/nixos)
              die "RAGOS runtime invalido: /boot ainda resolve para placeholder ($boot_device)"
              ;;
          esac
        }
        ...
        reexec_as_root() {
          ...
          if have_cmd sudo; then
            exec sudo --preserve-env=RAGOS_FLAKE_PATH,RAGOS_TARGET_HOST "$0" "$@"
          fi
        ...
        ensure_storage_dirs() {
          mkdir -p "$RAGOS_HOME_BASE" "$RAGOS_HOME_ARCHIVE_BASE" "$RAGOS_HOME_SNAPSHOT_BASE"
        }

        ensure_client_users_catalog() {
          mkdir -p "$RAGOS_RUNTIME_ROOT"
          if [[ ! -s "$RAGOS_CLIENT_USERS_FILE" ]]; then
            printf '{}\n' > "$RAGOS_CLIENT_USERS_FILE"
          fi
        }
        ...
        is_btrfs_home_storage() {
          [[ "$(findmnt -n -o FSTYPE --target "$RAGOS_HOME_BASE" 2>/dev/null || true)" == "btrfs" ]]
        }

        require_home_storage() {
          [[ -d "$RAGOS_HOME_BASE" ]] || die "storage de homes ausente em $RAGOS_HOME_BASE"
          mountpoint -q "$RAGOS_HOME_BASE" || die "home persistente nao esta montada em $RAGOS_HOME_BASE"
        }
        ...
        home_meta_path() {
          printf '%s/%s\n' "$(home_path_for_user "$1")" "$RAGOS_HOME_META_FILE"
        }
    ...
    EOF
        ...
        update_client_user_catalog() {
          ...
          jq \
            ...
            '
              ...
              | .[$user] = ($existing + {
                  uid: $uid,
                  description: ($existing.description // ("RAGOS User " + $user)),
                  hashedPassword: $hash,
                  extraGroups: $extraGroups,
                  groupGids: $groupGids
                })
            ' \
            "$RAGOS_CLIENT_USERS_FILE" > "$tmp_json"
          mv "$tmp_json" "$RAGOS_CLIENT_USERS_FILE"
          chmod 0644 "$RAGOS_CLIENT_USERS_FILE"
        }
        ...
        sync_client_user_catalog_entry() {
          ...
          uid="$(id -u "$user_name")"
          hashed_password="$(jq -r --arg user "$user_name" '.[$user].hashedPassword // empty' "$RAGOS_CLIENT_USERS_FILE" 2>/dev/null || true)"
        ...
        remove_client_user_catalog_entry() {
          ...
          ensure_client_users_catalog
          tmp_json="$(mktemp)"
          jq --arg user "$user_name" 'del(.[$user])' "$RAGOS_CLIENT_USERS_FILE" > "$tmp_json"
          mv "$tmp_json" "$RAGOS_CLIENT_USERS_FILE"
          chmod 0644 "$RAGOS_CLIENT_USERS_FILE"
        }
        ...
        enable_btrfs_quotas_if_needed() {
          if ! is_btrfs_home_storage; then
            die "storage de homes em $RAGOS_HOME_BASE nao e btrfs; quotas BTRFS indisponiveis"
          fi

          btrfs quota enable "$RAGOS_HOME_BASE" >/dev/null 2>&1 || true
        }
        ...
        current_generation() {
          local profile_link
          profile_link="$(readlink "$RAGOS_SYSTEM_PROFILE" 2>/dev/null || true)"
        ...
        run_nixos_rebuild() {
          ...
          local -a cmd=(env RAGOS_ENFORCE_RUNTIME_GUARDS=1 nixos-rebuild "$action")
          case "$action" in
            switch|test)
              cmd+=(--impure --flake "$RAGOS_INSTALLABLE")
              ;;
            *)
              ;;
          esac
        ...
        clean_without_nh() {
          ...
          local cutoff_epoch
          cutoff_epoch="$(date -d "$RAGOS_KEEP_SINCE_DATE_EXPR" +%s)"

          local -a generations=()
          while read -r generation stamp_date stamp_time _; do
            [[ "$generation" =~ ^[0-9]+$ ]] || continue
            generations+=("$generation|$stamp_date $stamp_time")
          done < <(nix-env --list-generations --profile "$RAGOS_SYSTEM_PROFILE" 2>/dev/null || true)

          local total="''${#generations[@]}"
          local keep_start=0
          if (( total > RAGOS_KEEP_GENERATIONS )); then
            keep_start=$(( total - RAGOS_KEEP_GENERATIONS ))
          fi
          ...
          if (( ''${#deletable[@]} > 0 )); then
            run_as_root nix-env --profile "$RAGOS_SYSTEM_PROFILE" --delete-generations "''${deletable[@]}"
          fi

          run_as_root nix-collect-garbage --delete-older-than "$RAGOS_KEEP_SINCE"
        ...
        cmd_sync() {
          ...
          git -C "$RAGOS_FLAKE_PATH" fetch --all --prune
          git -C "$RAGOS_FLAKE_PATH" pull --ff-only
          if [[ -f "$RAGOS_FLAKE_PATH/.gitmodules" ]]; then
            git -C "$RAGOS_FLAKE_PATH" submodule sync --recursive
            git -C "$RAGOS_FLAKE_PATH" submodule update --init --recursive
          fi
        }
        ...
        cmd_clean() {
          if have_cmd nh; then
            nh clean all --keep "$RAGOS_KEEP_GENERATIONS" --keep-since "$RAGOS_KEEP_SINCE" --optimise "$@"
            return 0
          fi
        ...
        cmd_repl() {
          require_flake_dir
          exec nix repl --expr "builtins.getFlake \"$RAGOS_FLAKE_PATH\""
        }
        ...
        cmd_user_add() {
          ...
          [[ -n "$user_name" ]] || die "uso: ragos user add <nome> --quota 20G"
          ...
          jq \
            ...
            "$RAGOS_USER_GROUPS_FILE" > "$tmp_json"
          mv "$tmp_json" "$RAGOS_USER_GROUPS_FILE"
        ...
        cmd_user_resize() {
          ...
          [[ -n "$user_name" ]] || die "uso: ragos user resize <nome> --quota 40G [--force]"
          quota="$(normalize_quota "$quota")"
          if (( force == 1 )); then
        ...
        cmd_user_list() {
          ...
          shopt -s nullglob
          for path in "$RAGOS_HOME_BASE"/*; do
            [[ -d "$path" ]] || continue
        ...
        cmd_user_delete() {
          ...
          [[ -n "$user_name" ]] || die "uso: ragos user delete <nome> --archive"
          ...
          archive_path="$RAGOS_HOME_ARCHIVE_BASE/$user_name-$stamp"

          if is_btrfs_home_storage; then
            btrfs subvolume snapshot -r "$home_path" "$RAGOS_HOME_SNAPSHOT_BASE/$user_name-$stamp" >/dev/null
          fi
          ...
          if [[ -d "$RAGOS_HOME_SNAPSHOT_BASE/$user_name-$stamp" ]]; then
            printf 'snapshot: %s\n' "$RAGOS_HOME_SNAPSHOT_BASE/$user_name-$stamp"
          fi
        }

        cmd_user_doctor() {
          local user_name="''${1:-}"
          [[ -n "$user_name" ]] || die "uso: ragos user doctor <nome>"
    ...
    quota: ''${quota:--}
    catalogo_cliente: $(jq -r --arg user "$user_name" 'if has($user) then "presente" else "ausente" end' "$RAGOS_CLIENT_USERS_FILE" 2>/dev/null || echo "ausente")
    montagem:
    ...
    EOF
        ...
        ensure_user_groups_catalog() {
          mkdir -p "$RAGOS_RUNTIME_ROOT"
          if [[ ! -s "$RAGOS_USER_GROUPS_FILE" ]]; then
            printf '{}\n' > "$RAGOS_USER_GROUPS_FILE"
          fi
        }

        ensure_storage_sectors() {
          mkdir -p "$RAGOS_STORAGE_BASE" "$RAGOS_STORAGE_ARCHIVE"
        }
        ...
        list_sector_storage() {
          ...
          printf '%-16s %-30s %s\n' "-----" "-------------" "---"
          shopt -s nullglob
          for sector_path in "$RAGOS_STORAGE_BASE"/*; do
        ...
        cmd_user_activity() {
          local user_name="''${1:-}"
          [[ -n "$user_name" ]] || die "uso: ragos user activity <nome>"
          ...
          jq --arg user "$user_name" '.sessions[$user] // []' "$RAGOS_AUDIT_FILE" 2>/dev/null | \
            jq -r '.[] | "[\(.timestamp)] [\(.action)] tty=\(.tty) ip=\(.ip)"'
        }

        cmd_group_add() {
          local group_name="''${1:-}"
          [[ -n "$group_name" ]] || die "uso: ragos group add <nome> [--description DESC] [--storage-quota 100G]"
          shift || true
          ...
          [[ -n "$description" ]] || description="RAGOS Group $group_name"
          quota="$(normalize_quota "$quota")"
          reexec_as_root group add "$group_name" --description "$description" --storage-quota "$quota"
          ensure_storage_sectors

          local group_storage="$RAGOS_STORAGE_BASE/$group_name"
    ...
    EOF
          ...
          jq \
            ...
            "$RAGOS_USER_GROUPS_FILE" > "$tmp_json"
          mv "$tmp_json" "$RAGOS_USER_GROUPS_FILE"
          chmod 0644 "$RAGOS_USER_GROUPS_FILE"
        ...
        cmd_group_delete() {
          local group_name="''${1:-}"
          [[ -n "$group_name" ]] || die "uso: ragos group delete <nome> --archive"
          shift || true
          ...
          mv "$group_storage" "$RAGOS_STORAGE_ARCHIVE/$group_name-$timestamp"

          # Remover do catalog
          ensure_user_groups_catalog
          local tmp_json
          tmp_json="$(mktemp)"
          jq --arg group "$group_name" 'del(.[$group])' "$RAGOS_USER_GROUPS_FILE" > "$tmp_json"
          mv "$tmp_json" "$RAGOS_USER_GROUPS_FILE"

          printf 'grupo arquivado: %s\narmazenamento em: %s\n' "$group_name" "$RAGOS_STORAGE_ARCHIVE/$group_name-$timestamp"
        }

        cmd_group_permissions() {
          local group_name="''${1:-}"
          [[ -n "$group_name" ]] || die "uso: ragos group permissions <nome>"
        ...
        cmd_group_chmod() {
          local group_name="''${1:-}"
          local perms="''${2:-}"
          [[ -n "$group_name" && -n "$perms" ]] || die "uso: ragos group chmod <nome> <perms> (ex: 0750 ou g+rw)"
        ...
        cmd_group_members() {
          local group_name="''${1:-}"
          [[ -n "$group_name" ]] || die "uso: ragos group members <nome> [--add USER] [--remove USER]"
          shift || true
    ...
    GROUP=admin
    DESCRIPTION=RAGOS Storage Admin - Permanent
    GID=3000
    QUOTA=1T
    CREATED_AT=$(date --iso-8601=seconds)
    PERMANENT=true
    ...
    GROUP=public
    DESCRIPTION=RAGOS Storage Public - Permanent
    GID=3001
    QUOTA=1T
    CREATED_AT=$(date --iso-8601=seconds)
    PERMANENT=true
    EOF
        ...
        cmd_group() {
          local action="''${1:-}"
          [[ -n "$action" ]] || die "uso: ragos group <add|list|delete|chmod|members|permissions|ensure-defaults> ..."
          shift || true
    ...
    plasma_look_and_feel_dirs: $(find /run/current-system/sw/share/plasma/look-and-feel -maxdepth 1 -mindepth 1 -type d 2>/dev/null | grep -E 'ragos|org\.ragos' | sed -n '1,20p' || true)
    plasma_desktoptheme_dirs: $(find /run/current-system/sw/share/plasma/desktoptheme -maxdepth 1 -mindepth 1 -type d 2>/dev/null | grep -E 'ragos|org\.ragos' | sed -n '1,20p' || true)
    plasma_color_schemes: $(find /run/current-system/sw/share/color-schemes -maxdepth 1 -type f -name 'RAGOS*.colors' 2>/dev/null | sed -n '1,20p' || true)
    plasma_wallpaper_dirs: $(find /run/current-system/sw/share/wallpapers -maxdepth 1 -mindepth 1 -type d 2>/dev/null | grep -E 'ragos|org\.ragos' | sed -n '1,20p' || true)
    EOF
        }
    ...
    inventario:
    $(sed -n '1,200p' /etc/ragos-inventory/clients.nix 2>/dev/null || true)
    EOF
        }
        ...
        cmd_status() {
          ...
          local runtime_root="$RAGOS_RUNTIME_ROOT"

          [[ -d "$RAGOS_FLAKE_PATH" ]] && directory_exists="sim"
          have_cmd nixos-rebuild && rebuild_available="sim"

          cat <<EOF
    flake_path: $RAGOS_FLAKE_PATH
    target_host: $RAGOS_TARGET_HOST
    ...
    Subcomandos:
      ...
      check      Executa nix flake check em /etc/ragos
      repl       Abre um REPL util para o flake local
      path       Mostra o caminho operacional do flake
      enter      Entra em /etc/ragos com shell interativo
    ...
    Contexto fixo:
      flake path: /etc/ragos
      runtime   : /var/lib/ragos/runtime
      host alvo : srv-rag

    Operacao de usuarios:
      ragos user add <nome> --quota 20G
    ...
    EOF
        ...
        cmd_user() {
          local action="''${1:-}"
          [[ -n "$action" ]] || die "uso: ragos user <add|resize|list|delete|doctor|quota-sync|activity> ..."
          shift || true
