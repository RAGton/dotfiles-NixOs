      ...
      assertions = [
        {
          ...
          message = "RAGOS: cada cliente do inventario precisa declarar mac, hostname e ip.";
        }
        {
          assertion = builtins.all (
            client: (!client ? channel) || builtins.elem client.channel allowedChannels
          ) inventory;
          message = "RAGOS: channel invalido no inventario (permitidos: generic, lab, rescue).";
        }
        {
          assertion = builtins.all (
            client: (!client ? releaseTrack) || builtins.elem client.releaseTrack allowedReleaseTracks
          ) inventory;
          message = "RAGOS: releaseTrack invalido no inventario (permitidos: stable, pilot, rescue).";
        }
        {
          assertion = invalidChannelTrackClients == [ ];
          message = "RAGOS: channel e releaseTrack incoerentes no inventario: ${lib.concatStringsSep ", " invalidChannelTrackClients}";
        }
        {
          assertion = builtins.all (
            client: (!client ? profile) || builtins.elem client.profile allowedProfiles
          ) inventory;
          message = "RAGOS: profile invalido no inventario (permitidos: desktop-generic, desktop-lab, hyperv-debug, rescue-minimal).";
        }
        {
          assertion = builtins.all (
            client: (!client ? clientProfile) || builtins.elem client.clientProfile allowedClientProfiles
          ) inventory;
          message = "RAGOS: clientProfile invalido no inventario (permitidos: workstation, lab-workstation, hyperv-debug, rescue).";
        }
        {
          assertion = invalidProfileAliasClients == [ ];
          message = "RAGOS: profile e clientProfile incoerentes no inventario: ${lib.concatStringsSep ", " invalidProfileAliasClients}";
        }
        {
          assertion = builtins.all (
            client: (!client ? bootMethod) || builtins.elem client.bootMethod allowedBootMethods
          ) inventory;
          message = "RAGOS: bootMethod invalido no inventario (permitidos: ipxe, uefi-http, uefi-https).";
        }
        {
          assertion = reservedBootMethods == [ ];
          message = "RAGOS: bootMethod=uefi-https ainda e reservado/futuro e nao pode ser usado sem prova real do caminho: ${lib.concatStringsSep ", " reservedBootMethods}";
        }
        {
          assertion = builtins.all (
            client: (!client ? hardwareClass) || builtins.elem client.hardwareClass allowedHardwareClasses
          ) inventory;
          message = "RAGOS: hardwareClass invalido no inventario (permitidos: physical-generic, physical-lab, hyperv, rescue).";
        }
        {
          assertion = invalidResolvedCombos == [ ];
          message = "RAGOS: combinacao invalida entre channel/releaseTrack, profile/clientProfile e hardwareClass: ${lib.concatStringsSep "; " invalidResolvedCombos}";
        }
        {
          assertion = duplicateMacs == [ ];
          message = "RAGOS: inventario invalido - MAC duplicado: ${lib.concatStringsSep ", " duplicateMacs}";
        }
        {
          assertion = duplicateHostnames == [ ];
          message = "RAGOS: inventario invalido - hostname duplicado: ${lib.concatStringsSep ", " duplicateHostnames}";
        }
        {
          assertion = duplicateIps == [ ];
          message = "RAGOS: inventario invalido - IP duplicado: ${lib.concatStringsSep ", " duplicateIps}";
        }
      ];
  ...
  validateInventoryWithPolicy =
    ...
    let
      ...
      nonEmptyAssertion = {
        ...
        message = "RAGOS: inventario externo vazio - recuse inventario vazio por padrao ou desabilite essa protecao explicitamente.";
      };
    in
    validated
    // {
