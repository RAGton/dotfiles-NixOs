resolve_rollback_target() {
  ...
  if [[ -n "$requested_target" ]]; then
    ...
    [[ -d "$IMAGES_ROOT/$requested_target" ]] || die "Versao nao encontrada: $requested_target. Use: ragc list"
    printf '%s\n' "$requested_target"
    return 0
  fi

  pointer_exists "$previous_pointer" || die "Nenhuma versao anterior encontrada em '$previous_pointer'. Use: ragc list"
  pointer_version "$previous_pointer"
}
...
cmd_rollback_impl() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --channel)
        shift
        [[ $# -gt 0 ]] || die "Uso: ragc rollback [VERSAO|previous] [--channel <generic|lab|rescue>]"
        requested_channel="$1"
        ;;
      --channel=*)
      ...
      "")
        ;;
      *)
        die "Argumento invalido para ragc rollback: $1"
        ;;
    esac
    shift || true
  done
...
cmd_rollback() {
  with_global_lock "ragc rollback" cmd_rollback_impl "$@"
}
