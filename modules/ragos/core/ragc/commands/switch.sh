cmd_switch_impl() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      --target)
        shift
        [[ $# -gt 0 ]] || die "Uso: ragc switch --target <desktop-generic|desktop-lab|hyperv-debug|rescue-minimal>"
        requested_target="$1"
        ;;
      --target=*)
      ...
      --channel)
        shift
        [[ $# -gt 0 ]] || die "Uso: ragc switch [--channel <generic|lab|rescue>] [--target <...>]"
        requested_channel="$1"
        ;;
      --channel=*)
      ...
      "")
        ;;
      *)
        die "Argumento invalido para ragc switch: $1"
        ;;
    esac
    shift || true
  done
  ...
  log_info "Buildando ragos-client-$resolved_target..."
  local system_path
  ...
  echo "  Target : $resolved_target"
  echo ""
  echo "  ragc rollback   - reverter se necessario"
}

cmd_switch() {
  with_global_lock "ragc switch" cmd_switch_impl "$@"
}
