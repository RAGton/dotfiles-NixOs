...
stdenvNoCC.mkDerivation {
  ...
  installPhase = ''
    ...
    themeDir="$out/share/sddm/themes/ragos-sugar-light"
    mkdir -p "$themeDir"
    cp -r ./* "$themeDir"/

    # Mantemos a base upstream do Sugar Light, mas precisamos preservar a
    # casca visual local do RAGOS para que o greeter nao volte ao visual
  ...
  meta = with lib; {
    description = "Tema SDDM Sugar Light adaptado para o branding do RAGOS";
