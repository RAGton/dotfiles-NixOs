...
let
  ...
  ragosBootSplash =
    pkgs.runCommand "ragos-installer-boot-splash.png"
      {
  ...
  ragosGrubTheme =
    pkgs.runCommand "ragos-installer-grub-theme"
      {
        nativeBuildInputs = [ pkgs.imagemagick ];
      }
      (normalizeText ''
...
in
{
  ...
  isoImage = {
    splashImage = ragosBootSplash;
    efiSplashImage = ragosBootSplash;
    grubTheme = ragosGrubTheme;
    syslinuxTheme = ''
      MENU TITLE RAGOS Installer
  ...
  services.getty.greetingLine = "\\e[1;34mRAGOS Installer\\e[0m";
  services.getty.helpLine = "";

  environment.etc."issue".text = ''

    \033[1;34mRAGOS Installer Live Environment\033[0m
    Web UI: http://<ip-da-maquina>:8000
    SSH: ragos@<ip-da-maquina> (senha: ragos)

  '';

  environment.etc."issue.net".text = "RAGOS Installer Live Environment\n";
}
